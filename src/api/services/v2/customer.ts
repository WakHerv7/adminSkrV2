...
import { customerUrlsV2, API_V2_BASE_URL } from "../../urls";
...
export class CustomerService {
	...
	static get_customers_stats = (params: any) => {
		...
		return BaseMethods.getRequest(
			customerUrlsV2.GET_CUSTOMERS_STATS,
			true,
			query_params
		);
	};
	static generate_customers_excel = (params: any) => {
		...
		// Call the admin v1v2 users Excel endpoint that returns streamed file data
		return BaseMethods.getRequest(
			// `${API_V2_BASE_URL}/v1v2/users/exports/v1v2-basic-multisheet?v1Limit=50000&v2Limit=50000`,
			// `${API_V2_BASE_URL}/v1v2/users/exports/users-monthly-stats`,
			`${API_V2_BASE_URL}/v1v2/users/exports/transactions-stats?category=card&type=payment_fee_failed`,
			// `${API_V2_BASE_URL}/v1v2/users/exports/transactions-stats?includedCategories=card.wallet&includedTypes=payment_fee.payment_fee_success.payment_fee_failed`,

			// `${API_V2_BASE_URL}/users/stats/monthly`,
			// `https://adminapi.getsekure.com/api/v2/admin/users/stats/monthly`,

			true,
			query_params
		);

		// 	includedCategories: textToArray(String('card.wallet')),
		//   includedTypes: textToArray(
		//     String('payment_fee.payment_fee_success.payment_fee_failed')
	};
	...
	static get_kyc_customers = (params: any) => {
		...
		return BaseMethods.getRequest(
			customerUrlsV2.GET_KYC_CUSTOMERS,
			true,
			query_params
		);
	};
	...
	static get_regularisations = (params: any) => {
		...
		return BaseMethods.getRequest(
			customerUrlsV2.GET_REGULARISATIONS,
			true,
			query_params
		);
	};
	static get_regularisation_stats = () => {
		return BaseMethods.getRequest(
			customerUrlsV2.GET_REGULARISATION_STATS,
			true
		);
	};
	...
	static update_one_customer = ({
		...
		return BaseMethods.putRequest(
			customerUrlsV2.UPDATE_ONE_CUSTOMER(customerId),
			body,
			true,
			query_params
		);
	};
	static update_one_customer_infos = ({
		userId,
		customerId,
		body,
	}: {
		...
		if (userId) query_params.adminUserId = userId;
		return BaseMethods.putRequest(
			customerUrlsV2.UPDATE_ONE_CUSTOMER_INFOS(customerId),
			body,
			true,
			query_params
		);
	};
	static update_one_customer_reg_status = ({
		userId,
		customerId,
		body,
	}: {
		...
		if (userId) query_params.adminUserId = userId;
		return BaseMethods.putRequest(
			customerUrlsV2.UPDATE_ONE_CUSTOMER_REG_STATUS(customerId),
			body,
			true,
			query_params
		);
	};
	static update_one_customer_password = ({
		userId,
		customerId,
		body,
	}: {
	...
	static get_stats_countries = ({
		...
		return BaseMethods.getRequest(
			customerUrlsV2.STATS_PER_COUNTRY,
			true,
			query_params
		);
	};
	...
	static handle_release_standby_balance = ({
		adminUserId,
		body,
	}: {
		adminUserId?: string;
		...
		return BaseMethods.patchRequest(
			customerUrlsV2.RELEASE_ONE_USER_BALANCE,
			body,
			true,
			query_params
		);
