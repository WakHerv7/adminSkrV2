...
const EditCountryModal: React.FC<EditCountryModalProps> = ({
	...
	const form = useForm<CountryFormData>({
		...
		defaultValues: {
			name: country?.name || "",
			phone_length: country?.phoneLength || undefined,
		},
	});

	// Réinitialiser le formulaire quand le pays change
	React.useEffect(() => {
		if (country) {
			form.reset({
				name: country.name,
	...
	const handleFormSubmit = (data: CountryFormData) => {
		const submitData = {
			...data,
			phone_length: data.phone_length || undefined,
		};
		onSubmit(submitData);
	};
				...
				<form onSubmit={form.handleSubmit(handleFormSubmit, onError)}>
					...
					<div className="flex gap-3 justify-end mt-8">
						...
						<button
							...
							disabled={isLoading}
						>
							{isLoading ? "Modification..." : "Modifier le pays"}
						</button>
					</div>
