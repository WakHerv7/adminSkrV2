...
const Transferts = ({search, setSearch}:Props) => {
  ...
  const trxPerStatus = transformArray(customerDetails?.transfers?.stats?.statuses ?? []);
  // console.log("trxPerStatus : ", trxPerStatus);

  const rearrangedTableData = customerDetails?.transfers?.data.map((item:any, index:any) => {
		let mode;
  ...
  return (
    <section className="p-6">
      <div className="w-full flex justify-start items-center">
        <div className="pr-10">
          <h1 className="text-sm font-semibold mb-2">Nombre de transferts<span className="text-base font-semibold text-[#18BC7A] ml-5">{customerDetails?.transfers?.stats?.totalCount ?? 0}</span></h1>
          <div className="flex justify-start items-center gap-10 w-full">
            <p title="Réussis" className=" flex items-center gap-2 text-sm font-normal">
              ...
              <span className="ml-1 font-bold">{trxPerStatus?.['null']?.count ?? 0}</span>
            </p>
            <p title="En cours" className=" flex items-center gap-2 text-sm font-normal">
              <IoEllipsisHorizontalCircleSharp size={24} color="#777"/>
              <span className="ml-1 font-bold">{customerDetails?.transfers?.stats?.totalPendingCount ?? 0}</span>
            </p>
            <p title="Echoués" className=" flex items-center gap-2 text-sm font-normal">
              <IoMdCloseCircle size={24} color="#F85D4B"/>
              <span className="ml-1 font-bold">{customerDetails?.transfers?.stats?.totalFailedCount ?? 0}</span>
            </p>
          </div>
        </div>
        {/* <Separator orientation="vertical" className="h-10 mr-10" /> */}
        <div className="h-full pt-2 border-l-1 px-10">
          <h1 className="text-sm font-bold mb-2">Total transferts</h1>
          <p className="text-xl font-bold text-[#18BC7A]">{customerDetails?.transfers?.stats?.totalAmount?.toLocaleString('fr-FR') ?? 0} XAF</p>
        </div>
        {/* <Separator orientation="vertical" className="h-10 mr-10" /> */}
        <div className="h-full border-l-1 px-10">
          <h1 className="text-sm font-bold mb-2">moyenne par semaine</h1>
          <p className="text-xl font-bold text-[#18BC7A]">{Math.round(customerDetails?.transfers?.stats?.avgAmount ?? 0)?.toLocaleString('fr-FR') ?? 0} XAF / sem</p>
        </div>
      </div>
