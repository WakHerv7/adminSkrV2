...
const handleBalanceWithdrawal = async (queryData: any) => {
	...
	const response = await RdcService.handle_rdc_balance_withdrawal({
		userId,
		phone: data?.phone,
...
export default function RetraitCDModalForm({ amount }: { amount: number }) {
	...
	const mutation = useMutation({
		...
		onSuccess: (data) => {
			...
			localStorage.setItem(
				`retraitRDC[${getCurrentDateTime()}]`,
				`Retrait de ${form.getValues(
					"amount"
				)} du solde RDC effectué avec SUCCES !`
			);
			toast.success(
				`Retrait de ${form.getValues(
					"amount"
