...
export function hasPermission(
 ...
 permission: string
    ...
    if(user?.role !== 'admin' || !user?.admin_role) {return false}
    
    switch (newarr.length) {
        case 1:
            hasPermision = (permissions?.[user?.admin_role]?.[newarr[0]]?.root)?.includes(permission);
            break;
        case 2:
            hasPermision = (permissions?.[user?.admin_role]?.[newarr[0]]?.[newarr[1]]?.root)?.includes(permission);
            break;
        case 3:
            hasPermision = (permissions?.[user?.admin_role]?.[newarr[0]]?.[newarr[1]]?.[newarr[2]]?.root)?.includes(permission);
            break;
        default:
            hasPermision = false
    }
