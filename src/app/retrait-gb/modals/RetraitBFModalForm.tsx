...
const handleBalanceWithdrawal = async (queryData: any) => {
	...
	const response = await BurkinaService.handle_burkina_balance_withdrawal({
		userId,
		phone: data?.phone,
...
export default function RetraitBFModalForm({ amount }: { amount: number }) {
	...
	const mutation = useMutation({
		...
		onSuccess: (data) => {
			...
			localStorage.setItem(
				`retraitBurkina[${getCurrentDateTime()}]`,
				`Retrait de ${form.getValues(
					"amount"
				)} du solde Burkina effectué avec SUCCES !`
			);
			toast.success(
				`Retrait de ${form.getValues(
					"amount"
