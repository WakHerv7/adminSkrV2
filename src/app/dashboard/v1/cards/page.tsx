...
import { cardDepositIconToday, cardDepositIconTotal, cardWithdrawalIconToday, cardWithdrawalIconTotal, closeCircleIcon, haltCircleIconGray, paymentIconToday, paymentIconTotal } from "@/constants/icons";
import {
	checkCircleIcon
} from "@/constants/Index";
import { useTitle } from "@/hooks/useTitle";
import { setCardList, setCardPayments, setCardPurchases, setCardTransactions } from "@/redux/slices/card";
...
export default function Cards() {
    ...
    if(allCardsStatsQueryRes?.data) {
		...
		infoData[0][0][0].value.text = (allCardsStatsQueryRes?.data?.totalAmount?.[0]?.totalAmount?.toLocaleString('fr-FR') ?? 0) + "  XAF";
		infoData[0][1][0].value.text = (allCardsStatsQueryRes?.data?.statuses?.['ACTIVE']?.count?.toLocaleString('fr-FR') ?? 0);
		infoData[0][1][1].value.text = (allCardsStatsQueryRes?.data?.statuses?.['FREEZE']?.count?.toLocaleString('fr-FR') ?? 0);
		infoData[0][1][2].value.text = (allCardsStatsQueryRes?.data?.statuses?.['DISABLED']?.count?.toLocaleString('fr-FR') ?? 0);
		infoData[1][0][0].value.text = (allCardsStatsQueryRes?.data?.recharge_carte?.todayTotal?.[0]?.todayTotalAmount?.toLocaleString('fr-FR') ?? 0) + "  XAF";
		infoData[1][1][0].value.text = (allCardsStatsQueryRes?.data?.recharge_carte?.avgTotal?.[0]?.totalAmount?.toLocaleString('fr-FR') ?? 0) + "  XAF";
		infoData[2][0][0].value.text = (allCardsStatsQueryRes?.data?.retrait_carte?.todayTotal?.[0]?.todayTotalAmount?.toLocaleString('fr-FR') ?? 0) + "  XAF";
		infoData[2][1][0].value.text = (allCardsStatsQueryRes?.data?.retrait_carte?.avgTotal?.[0]?.totalAmount?.toLocaleString('fr-FR') ?? 0) + "  XAF";
		infoData[3][0][0].value.text = (allCardsStatsQueryRes?.data?.payments?.todayTotal?.[0]?.todayTotalAmount?.toLocaleString('fr-FR') ?? 0) + "  XAF";
		infoData[3][1][0].value.text = (allCardsStatsQueryRes?.data?.payments?.avgTotal?.[0]?.totalAmount?.toLocaleString('fr-FR') ?? 0) + "  XAF";
			...
			<section className="my-10">
				<Tabs defaultValue="01" className="w-full">
					...
					<TabsContent value="02">
						<CardTransactions 
						isLoading={allTrxCardQueryRes.status == 'loading'}
					...
					<TabsContent value="03">
						<CardPurchases 
						isLoading={allCardPurchasesQueryRes.status == 'loading'}
					...
					<TabsContent value="04">
						<CardPayments
						isLoading={allCardPaymentsQueryRes.status == 'loading'}
