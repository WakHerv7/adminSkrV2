...
const handleBlock = async (queryData:any) => {
    const {currentUserId, cardId, customerId, body} = queryData;
    // console.log("handleTransaction : ", {currentUserId, customerId, label, body});
    // return {currentUserId, customerId, label, body}
    const response = await CardService.freeze_card({
        userId:customerId,
        cardId:cardId,
        body:{...body, id:currentUserId}
    }); 
    if (!response.ok) {
...
export default function BlockCardModalForm({index, card, customer}:ModalProps) {
    ...
    const mutation = useMutation({
		mutationFn: (data)=>handleBlock({currentUserId:currentUser?._id, cardId:card?.mapId, customerId:customer?._id, body:data}),
		onError: (err:any) => {
            console.error("onError : ", err.message);
            toast.error(`Erreur lors du blocage de la carte : ${err.message}`);		
		},
		onSuccess: (data) => {
            console.log("onSuccess : ", data);
            toast.success(`Blocage de la carte effectué avec succès`);
