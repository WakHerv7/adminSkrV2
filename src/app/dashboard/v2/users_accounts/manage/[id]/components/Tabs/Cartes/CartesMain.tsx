  ...
  return (
      <div style={{display:customerDetails?.cards?.data ? 'grid' : 'none'}}className="grid-cols-3 gap-10 mb-[100px]">
        {customerDetails?.cards?.data.map((carte:any, index:number) => {
          const cardXAFAmount = `${carte.balance_xaf?.toLocaleString('fr-FR') ?? 0} XAF`;
          const cardUSDAmount = `${retrieveUSDAmount({amount:carte?.balance_xaf, amountUSD:carte?.balance_usd})?.toLocaleString('fr-FR')} USD`;
          return <CartesCardV2
            key={index}
            index={index+1}
            card={carte}
            ...
            status={carte.status ?? ""}
            activateDate={getFormattedDateTime(carte.balance_updated_at) ?? ""}
