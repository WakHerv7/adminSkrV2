...
const ApprovedKyc = () => {
	...
	// Construire les paramètres de requête avec le statut fixe COMPLETED
	const queryParams = {
		status: "COMPLETED",
		...filterContent,
	};
	...
	const approvedKycQuery = useQuery({
		queryKey: ["approved-kyc", queryParams],
	...
	const kycData = Array.isArray(approvedKycQuery.data?.data)
		? approvedKycQuery.data?.data
		: approvedKycQuery.data?.data?.data || [];
