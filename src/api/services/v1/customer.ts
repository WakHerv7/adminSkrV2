...
import { customerUrlsV1 } from "../../urls";
...
export class CustomerService {
    static get_all_customers = (params:any) =>{
        ...
        return BaseMethods.getRequest(customerUrlsV1.GET_CUSTOMERS, true, query_params);
    }
    static get_stats_countries = ({limitDate}: {limitDate?:string}) => {
        let query_params:any = {};
        if(limitDate) query_params.limitDate =limitDate;        
        return BaseMethods.getRequest(customerUrlsV1.STATS_PER_COUNTRY, true, query_params);
    }
