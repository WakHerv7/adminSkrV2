...
export const formSchema:any = z.object({
    photo:  z.instanceof(FileList).refine(file => {        
...
const handleUpdate = async (queryData:any) => {
    ...
    console.log({bodyPhoto: body.photo[0]});
    const fileName = body.photo[0]?.name;
    const fileNewName = generateRandomCode(32);
    const fileExtension = getFileExtension(fileName);
    
    const formData = new FormData();
    formData.append("selfie", body.photo[0], `${fileNewName}.${fileExtension}`);

    const response = await UserService.update_user_selfie({
...
export default function UpdateSelfieModalForm({isOpen, setIsOpen, customer}:ModalProps) {
    ...
    const mutation = useMutation({
		...
		onError: (err:any) => {
            ...
            toast.error(`Erreur mise a jour du selfie : ` + err.message);		
		},
		onSuccess: (data) => {
            console.log("onSuccess : ", data);
            toast.success(`Le selfie a été mise à jour avec succès.`);
    ...
    return (
        ...
        title={`Mise à jour du selfie de`}
            ...
            <div className="min-w-[350px] max-w-[700px]">
                ...
                <form onSubmit={form.handleSubmit(onSubmit, onError)}>
                    <div className="w-full space-y-[20px]">
                        ...
                        render={({ field }) => (
                            <FormItem>
                            <FormLabel className="text-gray-900 text-sm font-[500] tracking-tight">Photo</FormLabel>
                            <FormControl>
                                <Input 
                                type="file" 
                        ...
                        />
                        {form.watch('photo')?.length > 0 && (
                            <Image
                            src={URL.createObjectURL(form.getValues('photo')[0])}
