...
const Transferts = ({ search, setSearch }: Props) => {
	...
	const rearrangedTableData = customerDetails?.transfers?.data?.map(
		(item: any, index: any) => {
			let mode;
			...
			<div className="w-full flex flex-col md:flex-row gap-10 md:gap-0 justify-start items-center">
				<div className="pr-10">
					<h1 className="text-sm font-semibold mb-2">
						...
						<span className="text-base font-semibold text-[#18BC7A] ml-5">
							{customerDetails?.transfers?.stats?.totalCount ?? 0}
						</span>
					</h1>
					<div className="flex justify-start items-center gap-10 w-full">
						...
						>
							...
							/>
							<span className="ml-1 font-bold">
								{customerDetails?.transfers?.stats
									?.totalPendingCount ?? 0}
							</span>
						</p>
						<p
				...
				<div className="h-full pt-2 border-l-1 px-10">
					...
					<p className="text-xl font-bold text-[#18BC7A]">
						{customerDetails?.transfers?.stats?.totalAmount?.toLocaleString(
							"fr-FR"
						) ?? 0}{" "}
						XAF
					</p>
				...
				<div className="h-full border-l-1 px-10">
					...
					<p className="text-xl font-bold text-[#18BC7A]">
						{Math.round(
							customerDetails?.transfers?.stats?.avgAmount ?? 0
