...
const handleUpdate = async (queryData:any) => {
    ...
    formData.append("photo", body.photo[0], `${fileNewName}.${fileExtension}`);

    const response = await UserService.update_user_photo({
...
export default function UpdatePhotoModalForm({isOpen, setIsOpen, customer}:ModalProps) {
    ...
    const mutation = useMutation({
		...
		onError: (err:any) => {
            ...
            toast.error(`Erreur mise a jour de la photo : ` + err.message);		
		},
		onSuccess: (data) => {
    ...
    return (
        ...
        title={`Mise à jour de la photo de`}
