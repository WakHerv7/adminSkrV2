...
// Schéma de validation Zod pour Service
const serviceSchema = z.object({
	...
	sens: z.enum(["IN", "OUT"], {
		required_error: "Le sens est requis",
		invalid_type_error: "Le sens doit être IN ou OUT",
	}),
	description: z
		.string()
	...
	transactionType: z.enum(
		["DEPOSIT", "WITHDRAWAL", "TRANSFER", "PAYMENT_LINK"],
		{
...
const CreateServiceModal: React.FC<CreateServiceModalProps> = ({
	...
	useEffect(() => {
		if (isEdit && initialData) {
			form.reset({
				...
				sens: initialData.sens || "IN",
				description: initialData.description || "",
				transactionType: initialData.transactionType || "DEPOSIT",
			});
		} else {
			form.reset({
				name: "",
	...
	// Options pour le champ sens
	const sensOptions = [
		{ value: "IN", label: "IN" },
		{ value: "OUT", label: "OUT" },
	];

	// Options pour le champ transactionType avec labels en français
	const transactionTypeOptions = [
		{ value: "DEPOSIT", label: "DEPOSIT (Dépôt)" },
		{ value: "WITHDRAWAL", label: "WITHDRAWAL (Retrait)" },
		{ value: "TRANSFER", label: "TRANSFER (Transfert)" },
		{ value: "PAYMENT_LINK", label: "PAYMENT_LINK (Lien de paiement)" },
	];

	// Version alternative avec juste les codes si préféré
	const transactionTypeSimpleOptions = [
			...
			<Form {...form}>
				<form onSubmit={form.handleSubmit(handleFormSubmit, onError)}>
					<div className="space-y-6">
								...
								<FormItem>
									...
									<FormControl>
										<CustomSelect
											...
											placeholder="Sélectionnez IN ou OUT"
											disabled={isLoading}
											error={!!form.formState.errors.sens}
										/>
									</FormControl>
									<FormMessage className="text-red-400 text-xs" />
								</FormItem>
						...
						{/* Champ Transaction Type avec Custom Select */}
						<FormField
							control={form.control}
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<CustomSelect
											options={transactionTypeOptions} // ou transactionTypeSimpleOptions
											value={field.value}
											onChange={field.onChange}
						...
						<FormField
							...
							render={({ field }) => (
								<FormItem>
									...
									<FormControl>
										<textarea
											placeholder="Ex: Money deposit service"
											{...field}
											disabled={isLoading}
