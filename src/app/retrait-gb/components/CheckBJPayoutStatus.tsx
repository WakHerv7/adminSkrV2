...
const getBeninBalance = async ({queryKey}:any) => {
  ...
  if (trxId) {
    ...
    const responseJson = await response.json();
    if (!response.ok) {
      throw new Error(responseJson.message || 'Failed to get_benin_payout_status'); 
    }  
    return responseJson.data; 
  } else {
...
export default function CheckBJPayoutStatus({trxId, trxStatus, setTrxStatus}:{
    trxId: string, 
    trxStatus?: string, 
  ...
  return (
    <div className='flex flex-col justify-center items-center my-[30px] w-full'>
        <div className='text-xl font-bold text-gray-900'>Payout pending</div>
