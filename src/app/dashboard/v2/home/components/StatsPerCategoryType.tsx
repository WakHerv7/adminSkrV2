	...
	if (!response.ok) {
		throw new Error(
			responseJson.message ||
				"Failed to get transactions per category and type"
		);
	}
...
export default function StatsPerCategoryType() {
	...
	const { dataData: cardsGraphData, trxData: cardsData } = getGraphData(
		transactionPerCategoryTypeQueryRes?.data?.cards ?? [],
		1
	);
	const { dataData: usersGraphData, trxData: usersData } = getGraphData(
		transactionPerCategoryTypeQueryRes?.data?.users ?? [],
		2
	);
	const { dataData: cardsTopUpsGraphData, trxData: cardsTopUpsData } =
		getGraphData(
			transactionPerCategoryTypeQueryRes?.data?.cardTopUpPerDay ?? [],
			3
		);
