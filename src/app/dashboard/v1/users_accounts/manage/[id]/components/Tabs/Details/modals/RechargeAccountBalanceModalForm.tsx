...
const handleTransaction = async (queryData:any) => {
    ...
    const response = await TransactionService.handle_user_account_transactions({
...
export default function RechargeAccountBalanceModalForm({customer}:TransferModalProps) {
    ...
    const mutation = useMutation({
		mutationFn: (data)=>handleTransaction({currentUserId:currentUser?._id, customerId:customer?._id, label:getFormLabels()?.label, body:data}),
		onError: (err:any) => {
		...
		onSuccess: (data) => {
            ...
            if(isRechargeAccount || isWithdrawAccount) {
                ...
                newCustomerDetails = {...customerDetails, customer:{...customer, soldeParrainage:data?.data?.updatedUser?.soldeParrainage}}
                dispatch(setCurrentCustomerDetails(newCustomerDetails));
            }
    ...
    const getFormLabels = (amount?:string | number) => {
        ...
        if (isRechargeSponsorshipAccount) {
            ...
            return {
                ...
                note:`(Montant maximal à retirer : ${customerDetails?.customer?.soldeParrainage})`,
                min:0,
                max:customerDetails?.customer?.soldeParrainage,
            }
        }
    }
