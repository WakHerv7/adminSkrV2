...
const handleUpdateUser = async (queryData:any) => {
  ...
  const response = await UserService.update_user_infos({
...
export default function Details() {
  ...
  const [isChangePasswordModalFormOpen, setIsChangePasswordModalFormOpen] = useState(false);
  const [isUpdatePhotoModalFormOpen, setIsUpdatePhotoModalFormOpen] = useState(false);
  const [isUpdateSelfieModalFormOpen, setIsUpdateSelfieModalFormOpen] = useState(false);
  const [isUpdateDocumentImagesModalFormOpen, setIsUpdateDocumentImagesModalFormOpen] = useState(false);
  ...
  const form = useForm<z.infer<typeof detailsSchema>>({
    ...
    defaultValues: {
      ...
      country: customerDetails?.customer?.pays ?? "",
      city: customerDetails?.customer?.city ?? "",
      address: customerDetails?.customer?.address ?? "",
      birthday: customerDetails?.customer?.birthday ?? "",
      age: calculateAge(parseDateStr(customerDetails?.customer?.birthday ?? "")), //String(customerDetails?.customer?.age ?? ""),
      job: customerDetails?.customer?.job ?? "",
      sex: customerDetails?.customer?.sex ?? "",
      income: customerDetails?.customer?.income ?? "",
      idExpDate: customerDetails?.customer?.idExpDate ?? "",
      codeParrain: customerDetails?.customer?.codeParrain ?? "",
      codeParrainage: customerDetails?.customer?.codeParrainage ?? "",
      deviceModel: customerDetails?.customer?.deviceModel ?? customerDetails?.customer?.deviceModeled ?? "",
      deviceID: customerDetails?.customer?.deviceID ?? "",
      idNumber: customerDetails?.customer?.idNumber ?? "",
      idPaper: customerDetails?.customer?.idPaper ?? "",
      tags: customerDetails?.customer?.tags ??[],
      additionalPhoneNumbers: customerDetails?.customer?.additionalPhoneNumbers ??[],
  ...
  const handleEditMode = (value: any) => {
    ...
    if(value) {
      ...
      form.setValue('name',customerDetails?.customer?.name ?? "");
      form.setValue('email',customerDetails?.customer?.email ?? "");
      form.setValue('phone',`${customerDetails?.customer?.phone}`);
      form.setValue('country_code',customerDetails?.customer?.country_code ?? "");
      form.setValue('country',customerDetails?.customer?.pays ?? "");
      form.setValue('city',customerDetails?.customer?.city ?? "");
      form.setValue('address',customerDetails?.customer?.address ?? "");
      form.setValue('codeParrain',customerDetails?.customer?.codeParrain ?? "");
      form.setValue('codeParrainage',customerDetails?.customer?.codeParrainage ?? "");
      form.setValue('deviceModel',customerDetails?.customer?.deviceModel ?? customerDetails?.customer?.deviceModeled ?? "");
      form.setValue('deviceID',customerDetails?.customer?.deviceID ?? "");
      form.setValue('idNumber',customerDetails?.customer?.idNumber ?? "");
      form.setValue('idPaper',customerDetails?.customer?.idPaper ?? "");
      form.setValue('age',calculateAge(parseDateStr(customerDetails?.customer?.birthday ?? "")));
      form.setValue('idExpDate',String(customerDetails?.customer?.idExpDate ?? ""));
      form.setValue('birthday',customerDetails?.customer?.birthday ?? "");
      ...
      form.setValue('additionalPhoneNumbers',customerDetails?.customer?.additionalPhoneNumbers ?? []);
    }
	};
            ...
            className="space-y-3">
              ...
              <div className="w-full flex flex-col gap-7">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="abc@sekure.xyz" className={`px-6 text-gray-900 font-normal ${isEditMode? 'bg-[#F4EFE3]': 'bg-gray-200'}`} {...field} readOnly={!isEditMode} />
                      </FormControl>
                    </FormItem>
                  )}
                />
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-900 text-sm font-semibold tracking-tight">Numéros de téléphone additionnels</FormLabel>
                      <FormControl>
                        {/* <Input className={`px-6 text-gray-900 font-normal ${isEditMode? 'bg-[#F4EFE3]': 'bg-gray-200'}`} {...field} readOnly={!isEditMode}/> */}
                        {isEditMode?
                          <TagField
                            ...
                            tags={field?.value}
                            onChange={(data) => {                              
                              form.setValue('additionalPhoneNumbers', data);
                            }}
                            max={2}
                          />
                          :
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-900 text-sm font-semibold tracking-tight">Age</FormLabel>
                      <FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-900 text-sm font-semibold tracking-tight">Code pays</FormLabel>
                      <FormControl>
                        <Input className={`px-6 text-gray-900 font-normal bg-gray-200`} {...field} readOnly />
                      </FormControl>
                    </FormItem>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-900 text-sm font-semibold tracking-tight">Code Parrainage</FormLabel>
                      <FormControl>
                        <Input className={`px-6 text-gray-900 font-normal bg-gray-200`} {...field} readOnly/>
                      </FormControl>
                    </FormItem>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-gray-900 text-sm font-semibold tracking-tight">Code Parrain</FormLabel>
                      <FormControl>
                        <Input className={`px-6 text-gray-900 font-normal bg-gray-200`} {...field} readOnly/>
                      </FormControl>
                    </FormItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-gray-900 text-sm font-semibold tracking-tight">ID Appareil</FormLabel>
                        <FormControl>
                          <Input className={`px-6 text-gray-900 font-normal bg-gray-200`} {...field} readOnly/>
                        </FormControl>
                      </FormItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className="text-gray-900 text-sm font-semibold tracking-tight">Tags</FormLabel>
                        <FormControl>
                          {/* <Input className={`px-6 text-gray-900 font-normal ${isEditMode? 'bg-[#F4EFE3]': 'bg-gray-200'}`} {...field} readOnly={!isEditMode}/> */}
                          {isEditMode?
                            <TagField
                  ...
                  <FormItem>
                    ...
                    {customerDetails?.customer?.idImages?.[0] ?
                        <Image
                            alt='vector background'
                            src={customerDetails?.customer?.idImages?.[0] }
                            layout='fill'
                            objectFit='contain'
                        />
                        :
                        <span className="pl-5 pt-5">Pas de recto</span> }                   
                    </div>
                  </FormItem>
                  <FormItem>
                    <FormLabel className="text-gray-900 text-sm font-semibold tracking-tight">{`VERSO Document d'identification`}</FormLabel>
                    <div style={{width: '100%', height: 400, borderRadius:'20px', position: 'relative'}}>
                        {customerDetails?.customer?.idImages[1] ?
                        <Image
                            alt='vector background'
                            src={customerDetails?.customer?.idImages[1] }
                            layout='fill'
                            objectFit='contain'
                        />
                        :
                        <span className="pl-5 pt-5">Pas de verso</span> }                  
                    </div>
                  </FormItem>
                  <FormItem>
                    <FormLabel className="text-gray-900 text-sm font-semibold tracking-tight">{`Selfie d'identification`}</FormLabel>
                    <div style={{width: '100%', height: 400, borderRadius:'20px', position: 'relative', overflow:'hidden'}}>
                        {customerDetails?.customer?.selfie ?
                        <Image
                            alt='vector background'
                            src={customerDetails?.customer?.selfie }
                            layout='fill'
                            objectFit='contain'
                        />
                      :
                      <span className="pl-5 pt-5">Pas de selfie</span>}                   
                    </div>
                  </FormItem>
