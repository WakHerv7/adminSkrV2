...
import { categoryType, categoryMode, getCategoryMode, getCategoryTypeV2, getCategoryModeV2 } from '@/utils/graphs';
  ...
  const rearrangedTableData = customerDetails?.transactions?.data?.map((item:any, index:any) => {
    ...
    const rearrangedItem = {
      ...
      mode: //item.mode, //item.paymentMethod,
        getCategoryModeV2(item.category, item.type, item.mode).mode == 'CREDIT'?
        <LabelWithBadge icon={<FaLongArrowAltDown color={'#18BC7A'} />} className={`text-xs`} label={`${getCategoryModeV2(item.category, item.type, item.mode).text}`} badgeColor={'#18BC7A'} textColor={'#444'}/>
        :
        getCategoryModeV2(item.category, item.type, item.mode).mode == 'DEBIT'?
        <LabelWithBadge icon={<FaLongArrowAltUp color={'#F85D4B'} />} className={`text-xs`} label={`${getCategoryModeV2(item.category, item.type, item.mode).text}`} badgeColor={'#F85D4B'} textColor={'#444'}/>
        :
        <>{item.mode}</>
        ,
			status: 
