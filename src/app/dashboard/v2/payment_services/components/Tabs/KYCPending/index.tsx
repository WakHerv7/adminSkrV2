...
import { headerKYCData, tableKYCData as data, headerKYCDataV2 } from "@/constants/KYCData";
import { selectKYCAll, selectKYCOngoing } from "@/redux/slices/kyc";
...
export default function Index({isLoading, search, setSearch}:Props) {
	...
	const kycAll:any = useSelector(selectKYCPending);
  	...
  	const rearrangedTableData = kycAll?.map((item:any, index:any) => {
		const rearrangedItem = {
			...
			status: item.kyc_result == 'APPROVED' 
